...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($data)) {
        $query = 'INSERT INTO products(img, name, description, price, price_after_discount, category_id, Dimensions,created_at) VALUES (?,?,?,?,?,?,?,NOW());';

        $stmt = $pdo->prepare($query);

        $success = $stmt->execute([$data['img'], $data['name'], $data['description'], $data['price'], $data['price_after_discount'], $data['category_id'], $data['Dimensions']]);
